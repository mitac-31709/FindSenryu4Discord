...
> 指定チャンネルで Discord のメッセージ検索を使い、過去の「川柳を検出しました！」返信とその返信元を辿って川柳をDBに取り込みます。`import.source_bot_ids` に旧Bot（例: 公式Bot）のユーザーIDを設定してください（未設定時は自分自身のBotのみ対象）。`dry_run:true` で保存せず件数確認できます。
...
公式Botや以前のインスタンスが残した検出返信を、チャンネル単位でDBへ取り込めます。全メッセージをページングするのではなく、Discord の Search Guild Messages（`川柳を検出しました！` + 取り込み元Bot + チャンネル）で候補を絞ります。
...
1. `config.toml` に取り込み元BotのユーザーIDを設定:
   ...
   ```
2. 対象チャンネルで「メッセージ履歴を読む」権限があること、Bot に `MESSAGE_CONTENT` intent があることを確認
3. 管理ギルドで実行:
   ```
...
重複は検出返信メッセージID（`source_message_id`）でスキップされます。返信元が削除済みの句は取り込みできません。検索 API の制約により、1回あたり最大約 10000 件までです。
...
- **履歴インポート** — `/admin import` でメッセージ検索により検出返信から川柳を再取り込み
