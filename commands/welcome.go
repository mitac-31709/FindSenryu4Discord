...
func buildWelcomeEmbed(tankaReaction string) *discordgo.MessageEmbed {
	return &discordgo.MessageEmbed{
		...
		Fields: []*discordgo.MessageEmbedField{
			...
			{
				Name:  "「詠め」「詠むな」",
				Value: fmt.Sprintf("「詠め」と発言するとサーバー内の川柳からランダムに一句詠みます。「短歌を詠め」で短歌（五・七・五・七・七）を詠みます。回数指定（「3回詠め」「3回短歌を詠め」）もできます。一句に %s リアクションを付けると短歌を新たに送ります。「詠むな」で直前の句を表示します。「詠んでてどう？」でよく使われた句やリアクション上位を表示します。", tankaReaction),
			},
			{
				Name:  "便利なコマンド",
